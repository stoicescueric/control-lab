/* Extended Kalman Filter tracking a target from noisy range + bearing radar.
   Linearises the nonlinear measurement with a Jacobian each step. Ported from
   legacy lessons/ekf.html; uses the shared lib/linalg.js. */

import { useRef, useState } from "react";
import { LA } from "../../lib/linalg.js";
...
export default function EKF() {
  ...
  const errMEl = useRef(null),
    errKEl = useRef(null);

  const st = useRef({ inited: false, x: null, P: null, t: 0, errM: 4, errK: 2, truePath: [], measPts: [], estPath: [] });
  const acc = useRef(0);
  ...
  function step() {
    ...
    const zr = rng + rNoise * randn(),
      zb = wrap(bear + bSd * randn());
    const mx = SX + zr * Math.cos(zb),
      my = SY + zr * Math.sin(zb);
