/* Odometry vs. truth. A robot drives a perfect circle (green); odometry rebuilt
   ...
   the estimate back. Ported from the legacy lessons/odometry.html. */
...
export default function OdometryField() {
  ...
  const roTrue = useRef(null),
    roEst = useRef(null),
    roErr = useRef(null);

  const st = useRef({ inited: false, tx: 0, ty: 0, tth: 0, ex: 0, ey: 0, eth: 0, t: 0, trueTrail: [], estTrail: [] });
  const acc = useRef(0);
  ...
  function step() {
    ...
    const p = { x: s.ex, y: s.ey, th: s.eth };
    odoStep(p, dLm, dRm);
    s.ex = p.x;
    s.ey = p.y;
    s.eth = p.th;
  ...
  function robot(cx, x, y, th, color) {
    cx.save();
    cx.translate(x, y);
    cx.rotate(th);
    cx.fillStyle = color;
