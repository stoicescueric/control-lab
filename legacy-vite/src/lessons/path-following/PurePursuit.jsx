/* Pure-pursuit path follower. The robot chases a "carrot" a fixed lookahead
   distance ahead on the path; drag the white waypoints to reshape it. Ported
   from the legacy lessons/path-following.html. */

import { useRef, useState } from "react";
import { useDprCanvas, useRaf } from "../../lib/canvas.js";
...
export default function PurePursuit() {
  ...
  function restart() {
    const s = st.current;
    const a = s.path[0],
      b = s.path[Math.min(8, s.path.length - 1)];
  ...
  function nearestAhead() {
    ...
    for (let i = s.pi; i < Math.min(s.path.length, s.pi + 80); i++) {
      const d = Math.hypot(s.path[i][0] - s.pose.x, s.path[i][1] - s.pose.y);
      if (d < bd) {
        bd = d;
        best = i;
      }
  ...
  function lookaheadPt() {
    ...
    for (let i = s.pi; i < s.path.length; i++) {
      if (Math.hypot(s.path[i][0] - s.pose.x, s.path[i][1] - s.pose.y) >= Ld) return s.path[i];
    }
  ...
  function step() {
    ...
    const dx = s.carrot[0] - s.pose.x,
      dy = s.carrot[1] - s.pose.y;
    const c = Math.cos(-s.pose.th),
      sn = Math.sin(-s.pose.th);
    const xr = dx * c - dy * sn,
      yr = dx * sn + dy * c;
  ...
  function evtPos(ev) {
    ...
    const cxp = ev.touches ? ev.touches[0].clientX : ev.clientX;
    const cyp = ev.touches ? ev.touches[0].clientY : ev.clientY;
