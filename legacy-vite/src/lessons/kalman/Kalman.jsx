/* 2-D constant-velocity Kalman tracker: turns noisy radar blips into a smooth
   estimate with an uncertainty bubble. Ported from legacy lessons/kalman.html. */
...
function makeAxis(p0) {
  return { x: [p0, 0], P: [[10, 0], [0, 10]] };
}

function kfStep(ax, z, q, r) {
  const [p, v] = ax.x;
  const pp = p + v * dt,
    pv = v;
  const P = ax.P;
...
export default function Kalman() {
  ...
  const st = useRef({ inited: false, axX: null, axY: null, t: 0, errM: 2, errK: 1, truePath: [], measPts: [], estPath: [] });
  const acc = useRef(0);
