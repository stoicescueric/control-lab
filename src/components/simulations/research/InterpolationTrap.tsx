...
export default function InterpolationTrap() {
  ...
  const toPath = (pts: Pt[]) =>
    'M ' +
    pts
      .map((p) => `${sx(p.x).toFixed(1)} ${sy(clamp(p.y, -20, Y_MAX + 20)).toFixed(1)}`)
      .join(' L ');
  ...
  const tickColor = (f: string) =>
    f === 'flat' ? '#5ce08a' : f === 'projected' ? '#ff6f9c' : '#8294b8';
    ...
    <>
      <svg
        ...
        <rect
          ...
          y={sy(dataMax)}
          width={sx(X_MAX) - sx(0)}
          height={sy(dataMin) - sy(dataMax)}
          fill="rgba(111,139,255,0.05)"
        />
