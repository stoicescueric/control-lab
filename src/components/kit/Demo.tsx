...
export function Stage({
  ...
  return (
    <div className={`grid gap-3.5 ${split ? 'md:grid-cols-2' : ''} ${className}`}>{children}</div>
  );
}
...
export function Button({primary = false, active = false, className = '', ...rest}: ButtonProps) {
  const tone = primary
...
export function Legend({
  ...
  items?: {color: string; label: ReactNode; dot?: boolean}[];
  className?: string;
}) {
  return (
...
/* A canvas styled for the demo panel. Forward the ref so sims can grab it. */
