  ...
  return (
    <Figure
      ...
      <svg
        viewBox="0 0 760 430"
        role="img"
        aria-label="Exploded view of a brushed DC motor with labelled components and the current path"
        ...
        <line
          ...
          strokeLinecap="round"
        />
        <text x={30} y={cy - 10} textAnchor="end" fontFamily={MONO} fontSize="13" fill={ROSE}>
          +
        </text>
        <text x={30} y={cy + 19} textAnchor="end" fontFamily={MONO} fontSize="13" fill={BLUE}>
          −
        </text>
        ...
        <Label
          ...
          lx={348}
          ly={cy - 116}
          text="armature (rotor): iron + windings"
          color={INK}
        />
        ...
        <path
          d={`M 452 ${cy - 66} A 20 66 0 0 1 488 ${cy - 66} A 20 42 0 0 0 470 ${cy - 44} A 20 42 0 0 0 452 ${cy - 66} Z`}
          fill={ROSE}
          opacity="0.9"
        />
        <path
          d={`M 452 ${cy + 66} A 20 66 0 0 0 488 ${cy + 66} A 20 42 0 0 1 470 ${cy + 44} A 20 42 0 0 1 452 ${cy + 66} Z`}
          fill={BLUE}
          opacity="0.9"
        />
        <text
        ...
        <Label
          ...
          lx={540}
          ly={cy - 122}
          text="magnets in the steel can (stator)"
          color={INK}
        />
        ...
        <line
          ...
          opacity="0.9"
        />
        <text x={552} y={cy + 2} fontFamily={MONO} fontSize="12" fill={GREEN}>
          field B
        </text>
        ...
        />
        <text x={64} y={cy - 30} fontFamily={MONO} fontSize="12" fill={AMBER}>
          current
        </text>
      </svg>
    </Figure>
  ...
  return (
    <Figure
      ...
      <svg
        viewBox="0 0 760 400"
        role="img"
        aria-label="Cross-sections of a brushed and a brushless DC motor"
        className="h-auto w-full">
        ...
        {[-90, 30, 150].map((a) => {
          ...
          return (
            <g key={a}>
              ...
              {/* pole shoe */}
              <path
                d={ringSector(L, cy, 72, 88, a - 24, a + 24)}
                fill="#4a5570"
                stroke="#7a87a6"
                strokeWidth="1"
              />
        ...
        {/* rotation arrow */}
        <path
          d={arcPath(L, cy, 138, -60, 20)}
          fill="none"
          stroke={GREEN}
          strokeWidth="2.5"
          markerEnd="url(#mGreen)"
        />
        <text x={L + 92} y={62} textAnchor="middle" fontFamily={MONO} fontSize="12" fill={GREEN}>
          rotor spins
        </text>

        <text x={L} y={cy + 152} textAnchor="middle" fontFamily={MONO} fontSize="12" fill={AMBER}>
          switched mechanically:
        </text>
        <text x={L} y={cy + 170} textAnchor="middle" fontFamily={MONO} fontSize="12" fill={AMBER}>
          brushes scrape the commutator
        </text>
        <text x={L} y={cy + 192} textAnchor="middle" fontFamily={MONO} fontSize="11" fill={MUTED}>
          windings spin · magnets fixed
        </text>
        ...
        {Array.from({length: 9}, (_, i) => {
          ...
          return (
            <g key={i}>
              <path
                d={ringSector(R, cy, 76, 116, a - 14, a + 14)}
                fill="#4a5570"
                stroke="#7a87a6"
                strokeWidth="1"
              />
        ...
        {[0, 90, 180, 270].map((a, i) => (
          <path
            key={a}
            d={ringSector(R, cy, 26, 50, a - 45, a + 45)}
            fill={i % 2 ? BLUE : ROSE}
            opacity="0.85"
          />
        ))}
        ...
        {/* rotation arrow */}
        <path
          d={arcPath(R, cy, 138, -60, 20)}
          fill="none"
          stroke={GREEN}
          strokeWidth="2.5"
          markerEnd="url(#mGreen)"
        />
        <text x={R + 92} y={62} textAnchor="middle" fontFamily={MONO} fontSize="12" fill={GREEN}>
          rotor spins
        </text>
        ...
        <rect
          ...
          strokeWidth="1.5"
        />
        <text x={R} y={cy + 157} textAnchor="middle" fontFamily={MONO} fontSize="12" fill={GREEN}>
          controller (ESC)
        </text>
        {PHASE.map(([, color], i) => {
          ...
          return (
            <path
              key={i}
              d={`M ${R - 30 + i * 30} ${cy + 140} C ${R - 30 + i * 30} ${cy + 118}, ${tx} ${ty + 16}, ${tx} ${ty}`}
              fill="none"
              stroke={color}
              strokeWidth="2.5"
              opacity="0.9"
        ...
        <text x={R} y={cy + 192} textAnchor="middle" fontFamily={MONO} fontSize="11" fill={MUTED}>
          magnets spin · no brushes to wear
        </text>
      </svg>
    </Figure>
  );
