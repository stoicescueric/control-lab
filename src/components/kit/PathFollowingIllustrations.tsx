  ...
  return (
    <Figure
      ...
      <svg
        viewBox="0 0 720 320"
        role="img"
        aria-label="Speed versus distance braking profile"
        ...
        <text x={x1} y={y0 + 24} fill={MUTED} fontFamily={MONO} fontSize="14" textAnchor="end">
          distance to target → (target at left)
        </text>
        <text x={x0 - 8} y={yTop} fill={MUTED} fontFamily={MONO} fontSize="14" textAnchor="end">
          speed
        </text>
        ...
        <g fontFamily={MONO} fontSize="14">
          ...
          <line
            ...
            strokeDasharray="9 7"
          />
          <text x={480} y={267} fill={ROSE}>
            pure kP·d (overshoots)
          </text>
        </g>
      </svg>
    </Figure>
  ...
  return (
    <Figure
      ...
      <svg
        viewBox="0 0 720 350"
        role="img"
        aria-label="Heading circle showing the angle-wrap seam"
        className="h-auto w-full">
        ...
        {/* seam marker */}
        <line
          x1={dial(cx, cy, r - 14, 180)[0]}
          y1={dial(cx, cy, r - 14, 180)[1]}
          x2={dial(cx, cy, r + 10, 180)[0]}
          y2={dial(cx, cy, r + 10, 180)[1]}
          stroke={AMBER}
          strokeWidth="2.5"
          strokeDasharray="4 4"
        />
        ...
        <path
          d={sampleArc(cx, cy, r + 0.0, A, B, 60)}
          fill="none"
          stroke={ROSE}
          strokeWidth="4"
          strokeLinecap="round"
          markerEnd="url(#awRose)"
          opacity="0.9"
        />

        {/* short way (+2° across the seam): 179° up to 181° (= −179°) */}
        <path
          d={sampleArc(cx, cy, r, A, 181, 10)}
          fill="none"
          stroke={GREEN}
          strokeWidth="6"
          strokeLinecap="round"
          markerEnd="url(#awGreen)"
        />
        ...
        <g fontFamily={MONO} fontSize="15">
          ...
          <text x={486} y={101} fill="#e8eefc">
            current = 179°
          </text>
          <circle cx={470} cy={130} r="6" fill={AMBER} />
          <text x={486} y={135} fill="#e8eefc">
            target = −179°
          </text>
          <line
            x1={458}
          ...
          <line
            ...
            strokeLinecap="round"
          />
          <text x={492} y={217} fill={ROSE}>
            naive error = −358°
          </text>
          <text x={458} y={262} fill={MUTED} fontSize="14">
            canonical result
          </text>
          <text x={458} y={286} fill="#e8eefc" fontSize="15">
            ((e + π) mod 2π) − π
          </text>
        </g>
      </svg>
  ...
  return (
    <Figure
      ...
      <svg
        viewBox="0 0 720 330"
        role="img"
        aria-label="PID-to-point error decomposed into robot-frame forward and strafe"
        className="h-auto w-full">
        ...
        <line
          ...
          strokeWidth="5"
          strokeLinecap="round"
          markerEnd="url(#pfBlue)"
        />
        <text x={O[0] + 116} y={O[1] - 6} fill={BLUE} fontFamily={MONO} fontSize="15">
          field error (eₓ, e_y)
        </text>
        ...
        <text x={T[0] + 14} y={T[1] + 2} fill={AMBER} fontFamily={MONO} fontSize="15">
          target pose
        </text>
        ...
        <g transform="translate(606 250)">
          <line
            ...
            stroke={MUTED}
            strokeWidth="3"
            markerEnd="url(#pfGray)"
          />
          <line
            x1="0"
            y1="0"
            x2={42 * Math.cos(rad(-12))}
            y2={-42 * Math.sin(rad(-12))}
            stroke={AMBER}
            strokeWidth="3"
            markerEnd="url(#pfAmber)"
          />
          <path d={sampleArc(0, 0, 24, phi, -12, 16)} fill="none" stroke={ROSE} strokeWidth="2.5" />
          <text x="-2" y="-48" fill={ROSE} fontFamily={MONO} fontSize="14" textAnchor="middle">
            e_θ
          </text>
          <text x="-2" y="64" fill={MUTED} fontFamily={MONO} fontSize="12" textAnchor="middle">
            heading error
          </text>
        </g>
      </svg>
  ...
  return (
    <Figure
      ...
      <svg
        viewBox="0 0 720 340"
        role="img"
        aria-label="Curvature continuity of cubic versus quintic splines"
        className="h-auto w-full">
        ...
        <path
          ...
          strokeLinecap="round"
        />
        <text x={x0} y={36} fill={MUTED} fontFamily={MONO} fontSize="13">
          the path (waypoints ●)
        </text>
        {[0, ...joins, 1].map((s, i) => {
        ...
        <line
          ...
          strokeDasharray="4 6"
        />
        <text x={x0} y={cy0 - 24} fill={MUTED} fontFamily={MONO} fontSize="13">
          curvature κ
        </text>
        ...
        {joins.map((s, i) => (
          <line
            key={i}
            x1={px(s)}
            y1={ky(quintic(s) + (i === 0 ? 0 : 0.5))}
            x2={px(s)}
            y2={ky(quintic(s) + (i === 0 ? 0.5 : -0.35))}
            stroke={ROSE}
            strokeWidth="2"
            strokeDasharray="2 4"
            opacity="0.8"
  ...
  return (
    <Figure
      ...
      <svg
        viewBox="0 0 720 340"
        role="img"
        aria-label="Guided vector field arrows converging onto a path"
        className="h-auto w-full">
        <rect width="720" height="340" rx="16" fill="#0b1120" />
        <PfArrowDefs />
        ...
        <line
          ...
          strokeLinecap="round"
        />
        <text x={664} y={yPath + 5} fill={GREEN} fontFamily={MONO} fontSize="14">
          path
        </text>
        ...
        <text x={R[0] + 58} y={R[1] - 8} fill={GREEN} fontFamily={MONO} fontSize="13">
          tangent t̂
        </text>
        <text
          x={R[0] - 10}
