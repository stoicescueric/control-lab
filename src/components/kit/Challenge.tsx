/* A sim-graded challenge chip. The sim owns the physics check and reports
   status/progress; this chip displays it, and persists a pass in the same
   local progress store as lesson completion, so a solved challenge stays
   solved across visits. */

import {useEffect, useState} from 'react';
import {isChallengePassed, passChallenge} from '@site/src/lib/progress';
...
export function ChallengeChip({
  ...
  return (
    <div
      className={`mt-4 flex flex-wrap items-center gap-3 rounded-xl border px-4 py-3 text-[0.86rem] transition-colors ${
      ...
      {passed ? (
        <span className="shrink-0 rounded-full bg-green/20 px-3 py-1 text-[0.75rem] font-bold text-green">
          Passed ✓
        </span>
      ) : (
