/* PID altitude-hold demo — a drone fighting gravity to a draggable target.
   The physics + drawing are ported from the legacy lesson; the canvas/animation
   hooks (lib/canvas) supply the loop, and per-frame UI (meters, readout) is
   updated imperatively via refs to avoid re-rendering 50× a second.
...
export default function Drone() {
  ...
  const barP = useRef<HTMLDivElement | null>(null);
  const barI = useRef<HTMLDivElement | null>(null);
  const barD = useRef<HTMLDivElement | null>(null);
  const numP = useRef<HTMLElement | null>(null);
  const numI = useRef<HTMLElement | null>(null);
  const numD = useRef<HTMLElement | null>(null);
  const roAlt = useRef<HTMLElement | null>(null);
  const roTgt = useRef<HTMLElement | null>(null);
  ...
  function setBar(el: HTMLDivElement | null, val: number) {
    if (!el) return;
  ...
  function pointToTarget(
    ...
    const ground = h - 24;
    const top = 20;
    const alt = ((ground - py) / (ground - top)) * AMAX;
