/* Velocity control: feedforward + PID vs PID-only, on a spin-up flywheel.
   Toggle FF to see the lag (and steady-state error) appear. */
...
export default function Flywheel() {
  ...
  const barFF = useRef<HTMLDivElement | null>(null);
  const barFB = useRef<HTMLDivElement | null>(null);
  const numFF = useRef<HTMLElement | null>(null);
  const numFB = useRef<HTMLElement | null>(null);
  const roV = useRef<HTMLElement | null>(null);
  const roT = useRef<HTMLElement | null>(null);
  const roE = useRef<HTMLElement | null>(null);
  const roVolt = useRef<HTMLElement | null>(null);
  ...
  function step() {
    ...
    const V2 = Math.max(-12, Math.min(12, kP * (goal - s.vPB)));
